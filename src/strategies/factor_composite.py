...
def generate_signals(df: pd.DataFrame, params: dict) -> pd.Series:
    ...
    Args:
        ...
        params: Keys:
            ...
            ``confidence_gain`` (default 0.0): scales the vol target up when the
              composite confidence is high, i.e. lever harder into confirmed
              uptrends ("let winners run") while exhaustion factors still cut size.
              Higher values raise NAV at the cost of some drawdown; 1.0 is a
              robust NAV preset (still beats BH on both metrics, all cells).
    ...
    # Confidence-scaled vol target: lever harder when the (positive) composite
    # confidence is high; exhaustion factors pulling the score down still de-risk.
